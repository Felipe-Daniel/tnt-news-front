...
export function Header({ user }: HeaderProps) {
  const [modalRecuperacaoSenhaVisivel, setModalRecuperacaoSenhaVisivel] =
    useState(false);
  ...
  return (
    <>
      <header className="flex justify-between bg-zinc-900 px-10 py-4 border-b border-b-black/50">
        <div className="flex items-center gap-5">
          ...
          <span className="text-white text-lg">
            Todas as notícias titânicas estão aqui!
          </span>
        </div>
        ...
        <div className="flex items-center gap-8">
          <div className="relative">
            ...
            <input
              ...
              className="w-80 pl-9 py-2 bg-zinc-800 rounded-md text-white"
            />
          </div>
          {!user ? (
            <Button
              text="ENTRAR"
              action={() => {
                setModalRecuperacaoSenhaVisivel(true);
              }}
            />
          ) : (
            <div className="flex items-center gap-8">
      ...
      <LoginModal
        visible={modalRecuperacaoSenhaVisivel}
        onCancel={() => setModalRecuperacaoSenhaVisivel(false)}
      />
    </>
  );
}
